...
def test_model(t_loader_test,iter_count,args): 
    if args.dataset_name == "OfficeHome":
        test_net = Alex_Model_OfficeHome().cuda()
    else:
        test_net = Alex_Model_Office31().cuda() 
...
def update_teacher(dataloader_no_shuffle,parser):
    ...
    if args.dataset_name == "OfficeHome":
        model = Alex_Model_OfficeHome().cuda()
    else:
        model = Alex_Model_Office31().cuda() 
    ...
    if args.dataset_name == "OfficeHome":
        teacher_feature = teacher_feature.reshape(-1,4096+65)
    else:
        teacher_feature = teacher_feature.reshape(-1,4096+31)
