...
parser.add_argument("--batch_size", help="set batch size", type=int, default=12)
...
parser.add_argument("--snapshot_file", help="generate folder to save models and result", \
    default='snapshot/'+'resnet_'+parser.parse_args().source_name+'_2_other_'+time )
...
if args.dataset_name == "OfficeHome":
    my_net = Res_Model_OfficeHome().cuda()
else:
    my_net = Res_Model_Office31().cuda()  
    ...
    with tqdm(total=args.max_iter, dynamic_ncols=True) as pbar: 
        for iter_count in range(args.max_iter):
            ...
            if args.dataset_name == "OfficeHome":
                ...
                vat_loss = VATLoss(xi=10.0, eps=1.0, ip=1)
                Lvir = vat_loss(my_net,s_imgs) + args.rho_val * vat_loss(my_net,t_imgs)
